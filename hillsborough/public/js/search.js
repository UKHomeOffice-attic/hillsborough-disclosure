...
// Redirect
		location.href = "/search/select?q="+keywords+params+"&rows=10&sourcesearch=adv";
	}	
}
...
function getURLVar(urlVarName)
{
	...
	for(i=0; i<=(urlVars.length); i++)
	{
		if(urlVars[i])
		{
			...
			if (urlVar.substr(0, urlVarName.length) == urlVarName)
			{
				return urlVar.substr(urlVarName.length);
			}
		}
	}
	return "";
